...
import { createIcon, PrimitiveIcon, IPrimitiveIcon } from "@gluestack-ui/icon";
...
const getIconStyle = (size: string = "md") => {
...
const Icon = React.forwardRef<React.ComponentRef<typeof UIIcon>, IIConProps>(
  function Icon({ size = "md", className, as: Component, ...props }, ref) {
    // If an external component is provided via 'as' prop, render it directly
    if (Component) {
      return (
        <Component
          ref={ref}
          {...props}
          className={getIconStyle(size) + (className ? ` ${className}` : "")}
        />
      );
    }
    ...
    if (typeof size === "number") {
      return (
        <UIIcon
          ref={ref}
          {...props}
          className={getIconStyle() + (className ? ` ${className}` : "")}
          size={size}
        />
      );
    } else if (
